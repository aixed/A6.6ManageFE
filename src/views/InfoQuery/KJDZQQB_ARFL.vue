<template>
  <div class="app-container">
    ...
    <b style="margin-left: 10px;">制单人ID：</b>
    <el-input
      type="text"
      v-model="zdr"
      style="width: 200px;"
      placeholder="可选：输入制单人ID"
    />
    ...
    >
      ...
      <el-table-column align="center" label="单位代码">
        <template slot-scope="scope">
          {{ scope.row.DWDM }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="账套名称" width="150">
        <template slot-scope="scope">
          {{ scope.row.ZTMC }}
        </template>
      </el-table-column>

      <el-table-column align="center" label="制单人ID" width="100">
        <template slot-scope="scope">
          {{ scope.row.ZDR }}
        </template>
      </el-table-column>

      <el-table-column align="center" label="制单人姓名" width="150">
        <template slot-scope="scope">
          {{ scope.row.ZDRXM }}
        </template>
      </el-table-column>

      <el-table-column align="center" label="凭证记账状态" width="140">
        <template slot-scope="scope">
          {{ scope.row.PZJZZT }}
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    cx(){
      ...
      this.axios({
        method: 'GET',
        url: vm.addr = api_Url + '/InfoQuery/dzqbk_ar',
        params: {
          nd: this.nd,
          dw: this.dw,
          zt: this.zt,
